...
class Hebrew implements DefinitionInterface
{
    ...
    public static function toInternal($date)
    {
        ...
        return Calends::toInternalFromUnix(BC::add(BC::mul(BC::sub(jewishtojd($greg->format('m'), $greg->format('d'), $greg->format('Y')), 2440587, 18), 86400, 18), BC::mod($greg->getTimestamp(), 86400, 18), 18));
    }
    ...
    public static function fromInternal($stamp)
    {
        ...
        list($m, $d, $y) = explode('/', jdtojewish(BC::add(BC::div($date, 86400, 18), 2440587.5, 18)));
